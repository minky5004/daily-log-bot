...
public final class Summarizer {
	...
	*/
	...
	private static final String RULES = """
			...
			- 근거와 배경은 `—` 뒤에 둔다
			- 명사구의 끝은 구체 명사다 — `~의 확인` `~의 부재` `~의 적용` 처럼 서술명사로 때우면 문장 구조가 그대로 남는다
			- 문체 예시(내용이 아니라 형태만 본다). 이렇게 — `캐시 계층 도입 — 매 요청 재계산하던 목록 · 응답 120ms에서 30ms로`
			...
			- 불릿 하나는 한 덩어리다 — 무엇을 했는지 · `—` 뒤에 왜 그렇게 했는지 · 본문에 실측값이나 검증 결과가 있으면 그중 하나
