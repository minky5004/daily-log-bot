...
public final class Summarizer {
	...
	*/
	...
	private static final String RULES = """
			...
			- 문장을 쓰지 않는다. 명사구로 끝낸다 — `~했다` `~한다` `~이다` `~합니다` 금지 · 어미만 명사형으로 바꾼 `~함` `~음` 도 금지
			- 연결어미로 잇지 않고 `·` 로 병치한다 — `~하고` `~이고` `~라` `~므로` `~면` `~아서` `~기 위해` `~기 위함` `~을 위한` `~용` 이 남으면 그것은 문장이다
			- `~해` `~어` `~여` 로 이어 붙이는 것도 문장이다 — `옮겨` `고쳐` `늘려` `줄여` `맞춰`
			- 병치 기호는 `·` 하나뿐이다. `및` `와` `과` 로 잇지 않는다 — `A 및 B` `A 와 B` 는 언제나 `A · B` · 병치가 아닌 `원본과 일치` 같은 자리는 그대로 둔다
			- 근거와 배경은 `—` 뒤에 둔다
			- 명사구는 구체 명사로 끝낸다. `해소` `해결` `방지` `완화` `통제` `개선` `확인` `적용` `부재` `보존` `결과` 처럼 뜻이 빈 서술명사로 끝내지 않는다 — 그 자리는 관형절로 돌려 구체 명사를 수식하게 한다
			- `표 사이 수치 불일치 해소` 가 아니라 `기술 스택 표 · 검증 표에서 갈리던 수치`
			- `문서 수정 부담 완화` 가 아니라 `코드가 바뀔 때마다 따라 고치던 수치`
			- `렌더링 이슈 해결` 이 아니라 `방문자마다 갈리던 날짜 표기`
			- 문체 예시(내용이 아니라 형태만 본다). 이렇게 — `캐시 계층 도입 — 매 요청 재계산하던 목록 · 응답 120ms → 30ms`
			- 이렇게 쓰지 않는다 — `캐시 계층을 도입했다. 매 요청마다 목록을 재계산하는 문제를 해결하기 위함이다.`
			...
			- 실측값은 그것이 적힌 저장소의 절에만 싣는다 — PR 본문이 다른 저장소의 수치를 인용한 자리는 옮기지 않는다
			...
			- 다 쓴 뒤 불릿마다 되짚는다 — `및` 이 남았는가 · `~해` `~어` `~고` 로 이어 붙였는가 · 서술명사로 끝났는가 · 활동 본문의 실측값을 빠뜨렸는가 · 다른 저장소의 수치를 옮겨 왔는가
	...
	private ObjectNode schema() {
		...
		properties.putObject("summary").put("type", "STRING").put("description", "하루를 가리키는 한 줄");
