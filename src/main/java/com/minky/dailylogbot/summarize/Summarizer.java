...
public final class Summarizer {
	...
	*/
	...
	private static final String RULES = """
			...
			- 연결어미로 잇지 않고 `·` 로 병치한다 — `~하고` `~이고` `~라` `~므로` `~면` `~아서` `~기 위해` `~기 위함` 이 남으면 그것은 문장이다
			...
			- summary 는 한 줄 · 100자 이내 · 그날을 통째로 가리키는 명사구
			- body 는 마크다운 · 저장소마다 `## 저장소명` · 그 아래 `-` 불릿 · 전체 2000자 이내
			- 불릿 하나는 한 덩어리다 — 무엇을 했는지 · `—` 뒤에 왜 그렇게 했는지
			- 커밋 메시지 · PR 본문의 글에 적힌 실측값이나 검증 결과는 그 수치를 그대로 싣는다 — `테스트 276개 통과` `응답 120ms` 처럼 · 활동 목록 머리의 `+n -n` 과 건수는 여기 해당하지 않는다
